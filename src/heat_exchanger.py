...
def pitch_from_tubes(self, tubes, pattern):
...
class Heat_Exchanger():
    def __init__(self, cold_fluid_path, hot_fluid_path, flow_path_entries_side):
        ...
        self.pitch = pitch_from_tubes(self, self.total_tubes, pattern)
    ...
    def LMTD_heat_solve_iteration(self, Tout):
        ...
        area_times_H = self.calc_area_times_H(self.mdot)
        ...
        cold_eq = mdot_cold * cp * \
            (T1out - T1in) - area_times_H * Fscale * \
            logmeanT(T1in, T1out, T2in, T2out)
        hot_eq = mdot_hot * cp * \
            (T2in - T2out) - area_times_H * Fscale * \
            logmeanT(T1in, T1out, T2in, T2out)
    ...
    def compute_effectiveness(self, method = "LMTD"):
        ...
        if (method == 'LMTD'):
            ...
            effectiveness = Qdot / (mdot_cold * cp * (T2in - T1in))
            ...
            # TODO: Ask if this requires Fscale ?
            NTU = Area_times_H / C_min
    ...
    def set_geometry(self, length, tubes, baffles):
        ...
        self.pitch = pitch_from_tubes(self, tubes, pattern)
